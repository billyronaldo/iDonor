    ...
    checkLocation = (locationOn) => {
      ...
      buildPanels() { 
          if (this.state.requests.length > 0 && this.state.toggled === true) {
            return this.state.requests.filter(element => {
              let cekDarah = this.check(this.props.currentUser.golDarah, element.golDarah) && element.geo_point.latitude 
              ...
              let distance = this.calculateDistance(this.props.currentUser.geo_point.latitude, this.props.currentUser.geo_point.longitude,
                      element.geo_point.latitude , element.geo_point.longitude); console.log(element.golDarah, cekDarah);
              if(cekDarah === true){
                ...
                <View style={{flex: 1}}>
                    ...
                    <View style={styles.panelRow}>
                      ...
                      <TouchableOpacity style={{flex: 0.5}} onPress={()=>{Linking.openURL(`tel:${req.noHp}`)}}>
                    ...
                    <View style={styles.panelRow}>
                      ...
                      <View style={{flexDirection: 'row', alignSelf:'flex-end'}}>
                          <TouchableOpacity onPress={()=>{Linking.openURL(`tel:${req.noHp}`)}}>
          ...
          else if(this.state.requests.length === 0 && this.state.toggled === true){
            return <View style={{alignItems: 'center'}}>
              <Text style={{fontSize: fontSize.regular + 1}}>Tidak ada request saat ini</Text>
            </View>
          }
          else if(this.state.toggled === false){
            return <View style={{alignItems: 'center'}}>
              <Text style={{fontSize: fontSize.regular + 1}}>Anda belum bersedia menjadi donor</Text>
            </View>
          }     
  }
        ...
        return(
            <View style={styles.container}>
              <View style={{flexDirection:'row', justifyContent:'space-evenly'}}>
                <Text style={{fontSize: fontSize.regular + 2}}>Bersedia menjadi pendonor</Text>
