...
class Profile extends Component {
  ...
  fetchRequests(){
   this.subscriber = firebase.firestore()
    .collection("requests").doc(firebase.auth().currentUser.uid).onSnapshot(doc => {
      if(doc.exists){
        this.setState({
          requests: {
            name: doc.data().name,
...
deleteData(){
  firebase.firestore().collection("requests")
    .doc(firebase.auth().currentUser.uid).delete().then((res) => {
      console.log(res)
      this.setState({
        requests:{}
      })
...
buildPanels() { 
  ...
  if (this.state.requests.name !== null) {
        ...
        <View style={{flex: 1}}>
          <Text style={{fontSize: fontSize.regular+1, color: color.red}}>Request Donor Anda</Text>
            ...
            <View style={styles.panelRow}>
              ...
              <View style={{flexDirection: 'row', alignSelf:'flex-end'}}>
                  <TouchableOpacity onPress={this.deleteData}>
                      <Text style={{color:color.red, fontSize: fontSize.regular}}>Akhiri Request</Text>
                  </TouchableOpacity>
              </View>
            </View>
        </View>
          ...
          <View style={styles.container}>
            <Text style={{fontSize: fontSize.large + 1}}>Profile</Text>
                 <View style={{marginHorizontal: normalize(20)}}>
                   <Text style={styles.textStyle}>Nama          : {this.props.currentUser.name}</Text>
                   <Text style={styles.textStyle}>No HP         : {this.props.currentUser.noHp}</Text>
                   <Text style={styles.textStyle}>Jenis Kelamin : {this.props.currentUser.gender}</Text>
                   <Text style={styles.textStyle}>Golongan Darah: {this.props.currentUser.golDarah}</Text>
                 </View>
                 <TouchableOpacity onPress={() => Actions.Information()}>
                  <Text style={{color: color.red, fontSize: fontSize.regular}}>Lihat Syarat Donor Darah</Text>
                 </TouchableOpacity>
