      ...
      return (
        <View style={styles.container}>
          <MapView  
            ...
            style={styles.map}
            initialRegion={{latitude: this.props.currentUser.geo_point.latitude, longitude: this.props.currentUser.geo_point.longitude, latitudeDelta:0.0922, longitudeDelta: 0.0421}}>
            {this.state.users.filter(element => {
              let cekDarah = this.check(this.state.golDarah, element.golDarah) && element.geo_point.latitude 
            ...
            coordinate={{latitude: user.geo_point.latitude, longitude: user.geo_point.longitude}}>
              <Callout onPress={()=>{Linking.openURL(`tel:${user.noHp}`)}}>
                <View style={{height: 0.15 * windowHeight(), width: 0.4 * windowWidth()}}>
